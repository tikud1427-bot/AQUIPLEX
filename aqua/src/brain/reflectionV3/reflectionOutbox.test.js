...
import { toContradictionJob, isClaimContradictionEvent, contradictionJobKey } from './reflectionOutbox.js';
...
const calls = [];
const testMind = { beliefs: {}, reflectionLedger: {} };
...
const contradictionEvent = { outboxId: 77, ownerId: 'user:a', eventType: 'claim.contradiction.detected', aggregateId: 'new', payload: { incomingClaimId: 'new', existingClaimId: 'old', kind: 'polarity' } };
assert.equal(isClaimContradictionEvent(contradictionEvent.eventType), true);
assert.equal(toContradictionJob(contradictionEvent).kind, 'claim.contradiction.v1');
assert.equal(toContradictionJob(contradictionEvent).idempotencyKey, contradictionJobKey(contradictionEvent));
console.log('E9 contradiction outbox bridge: 3/3 assertions passed');

assert.equal(isClaimReflectionEvent('claim.contradiction.detected'), false);
assert.equal(toContradictionJob({ ...contradictionEvent, payload: { ...contradictionEvent.payload, incomingClaimId: 'new', existingClaimId: 'old' } }).payload.incomingClaimId, 'new');
console.log('E9 contradiction/reflection event separation: 2/2 assertions passed');
